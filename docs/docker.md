...
The `docker-compose.yml` in this repo uses the pre-built image by default — no local build required.

## Build locally (if you need to)
...
PipeWire exposes an ALSA compatibility layer, so `/dev/snd` still works. To force a specific capture device (e.g. a webcam mic), set the ALSA card name:

```bash
# Find your card name on the host
arecord -l
