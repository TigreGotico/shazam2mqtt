...
All settings are exposed as environment variables. There is no config file.
...
| `NOISE_GATE_DB` | `-40` | dBFS threshold. Audio quieter than this is silence. |
...
- **False triggers:** lower the value (e.g. `-50`) or check that the mic is not picking up fan noise.

## Hysteresis

The gate requires **3 consecutive loud seconds** before triggering. This is hard-coded to avoid reacting to single pops or keyboard clicks.
